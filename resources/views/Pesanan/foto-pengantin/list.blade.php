<x-backend-layout>
  ...
  <div class="card border-top-info">
    <div class="card-header">
      Daftar Pesanan Foto Pengantin
    </div>
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-bordered" endpoint="{{route('pesanan.foto-pengantin.datatable')}}" width="100%" id="pesanan-foto-pengantin" cellspacing="0">
          <thead>
            <tr>
              <th>#</th>
              <th>No Pesanan</th>
  ...
  <script>
    (() => {
      const dataTablePesanan = $("#pesanan-foto-pengantin").DataTable({
        processing: true,
        "searching": false,
        serverSide: true,
        ajax: {
          url: $("#pesanan-foto-pengantin").attr('endpoint'),
          "dataSrc": function(json) {
            const data = json.data.map(psn => {
              let btn = `<a href="/admin/pembayaran/foto-pengantin/${psn.no_pesanan}" class="btn btn-primary">Pembayaran</a>`;
              if (psn.status === 1) {
                btn = `<a href="/admin/pembayaran/foto-pengantin/${psn.pembayaran.no_invoice}/invoice" class="btn btn-info">Invoice Pesanan</a>`;
              }
              psn.button = btn;
              return psn;
            })
